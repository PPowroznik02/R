...
par(mar = c(4,3,4,3))
plot(x,y, type='l', col="black", main="Minimalny zasiêgu lodu morskiego dla ka¿dego z k¹tów", xlim = c(-80,80), ylim = c(-80,80), xlab="", ylab="", axes=FALSE, frame.plot=TRUE)
#lines(x1,y1, col="red")
#legend("topright", legend=c("minima zasiêg lodu", "wymodelowany zasiêg lodu"), col=c("black", "red"), lty=c(1,1), cex=0.8)
dev.off()
...
p <- p %>% layout(scene = list(xaxis = list(title = 'czas'),
                               yaxis = list(title = 'dlugosæ geograficzna'),
                               zaxis = list(title = 'szerokosæ geograficzna')))
p
...
for (k in 1:nrow(allDataEveryTwoDays))
{
  ...
  par(mar = c(4,3,4,3))
  plot(x,y, type='l', col="black", main="Zmiana zasiêgu lodu morskiego na Antarktydzie w latach 1978 - 2009", xlim = c(-80,80), ylim = c(-80,80), xlab="", ylab="", axes=FALSE, frame.plot=TRUE)
  lines(x1,y1, col="red")
  legend("topright", legend=c("rzeczywisty zasiêg lodu", "wymodelowany zasiêg lodu"),
